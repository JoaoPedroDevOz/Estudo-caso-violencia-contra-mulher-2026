...
# print(df[(df['dt_nasc'].isnull()) & (df['nu_idade_n'].isnull())]) # As três linhas com nu_idade_n nulos estão com dt_nasc nulos também, pode-se desconsiderar essas linhas por conter os dados "faltoso", assim sendo deletadas.

df = df.dropna(subset=['nu_idade_n'])

# print(df['nu_idade_n'].isnull().sum()) # Resp: 0

print(df['dt_nasc'].isnull().sum())

## Segunda parte é popular a coluna dt_nasc a partir do nu_idade_n, fazendo o cálculo do ano subtraido pelo valor
...
df_dt_notific = (
    df.groupby([
        df["dt_notific"].dt.year.rename("ano"),
        df["dt_notific"].dt.month.rename("mes")
    ])
    .size()
